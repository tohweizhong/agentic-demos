...
Designed for standard setups: scans the "global" location and resolves standard Google Workspace email creator identities.
...
def get_agent_creators(session, project_id, agent_ids, min_create_time=None, timeout=30):
    """Retrieves agent creator emails from Cloud Audit Logs for specific agent IDs."""
    if not agent_ids:
        return {}
    ...
    while True:
        ...
        try:
            ...
            for entry in entries:
                ...
                if agent_name and principal_email:
                    ...
                    if agent_id and agent_id != "default_assistant" and agent_id not in creators:
                        creators[agent_id] = principal_email
            next_page_token = data.get("nextPageToken")
            if not next_page_token:
                break
        except Exception as e:
...
def get_payload_email(agent, agent_type):
    ...
    # Only return if it looks like a valid email address and is not a SPIFFE ID
    if payload_author and "@" in payload_author and not payload_author.startswith("SPIFFE"):
